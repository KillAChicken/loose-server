...
def test_add_rule(core_manager, server_rule_prototype):
    ...
    assert actual_rule is server_rule_prototype, "Different rule is returned"


def test_add_several_rules(core_manager, server_rule_prototype):
    ...
    """
    first_rule = server_rule_prototype.create_new(rule_type="first")
    first_rule_id = core_manager.add_rule(rule=first_rule)

    second_rule = server_rule_prototype.create_new(rule_type="second")
...
def test_id_collision(core_manager, server_rule_prototype, monkeypatch):
    ...
    first_rule_id = core_manager.add_rule(server_rule_prototype)
    second_rule_id = core_manager.add_rule(server_rule_prototype)

    assert first_rule_id == "1", "Wrong rule ID"
    assert second_rule_id == "2", "Wrong rule ID"


def test_get_non_existent_rule(core_manager, server_rule_prototype):
    ...
    """
    core_manager.add_rule(rule=server_rule_prototype)
...
def remove_rule(core_manager, server_rule_prototype, rule_index):
    ...
    """
    rule_ids = [core_manager.add_rule(rule=server_rule_prototype) for _ in range(3)]
...
def test_rules_order_after_add(core_manager, server_rule_prototype, number_of_rules):
...
def test_rules_order_after_remove(core_manager, server_rule_prototype, number_of_rules):
